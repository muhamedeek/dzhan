...
// --- Minimal UI primitives (replace missing ./ui-stubs) ---
// These are lightweight stand-ins for Button/Card components.
// If you later add shadcn/ui or your own UI kit, swap these out.
...
export const Button: React.FC<ButtonProps> = ({ className, variant = "default", ...props }) => (
  <button
    className={cx(
      ...
      variant === "default" && "bg-white/10 hover:bg-white/20 border border-white/15 text-white",
      variant === "ghost" && "bg-transparent hover:bg-white/10 border border-white/10 text-white/80",
      className
    )}
    {...props}
  />
...
export const Card: React.FC<DivProps> = ({ className, ...props }) => (
  <div className={cx("rounded-xl border border-white/10 bg-white/5", className)} {...props} />
);
...
export default function PortfolioPage() {
  ...
  // --- Сумма: «сырой» ввод + валидное число для расчётов + ошибка
  ...
  // Коммит значения из текстового поля в расчётную сумму
  function commitAmount() {
    const raw = amountInput.trim();
    if (raw === "") {
      setAmountError(lang === "ru" ? "Введите сумму" : "Enter amount");
      return;
    }
    const v = Number(raw);
    if (Number.isNaN(v)) {
      setAmountError(lang === "ru" ? "Некорректное число" : "Invalid number");
      return;
    }
    if (v < 500000) {
      setAmountError(lang === "ru" ? "Минимальная сумма инвестиций — 500 000 сом" : "Minimum investment is 500,000 KGS");
      return;
    }
    setAmountError(null);
    setAmount(v);
            ...
            <CardContent className="pt-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.currentTarget.blur(); // триггерим commitAmount через onBlur
                    }
                  }}
                  className={`w-full rounded-xl bg-white/10 border px-4 py-3 outline-none text-white placeholder-white/40 text-lg font-semibold
                    ${amountError ? "border-red-500/60 focus:border-red-400" : "border-white/10 focus:border-blue-500"}`}
                  placeholder={lang === "ru" ? "Введите сумму" : "Enter amount"}
                />
                {amountError && (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setMode("reinvest")} className={cx("rounded-xl px-4 py-3 border", mode === "reinvest" ? "bg-blue-600 text-white border-blue-500 shadow-[0_0_25px_#2563eb]" : "bg-white/10 text-white/80 border-white/10 hover:bg-white/15")}> 
                    {t.modeReinvest}
                  </button>
                  <button onClick={() => setMode("payout")} className={cx("rounded-xl px-4 py-3 border", mode === "payout" ? "bg-blue-600 text-white border-blue-500 shadow-[0_0_25px_#2563eb]" : "bg-white/10 text-white/80 border-white/10 hover:bg-white/15")}>
                    {t.modePayout}
                  </button>
                </div>
              </div>
...
// --- Lightweight tests (won't break UI). Open console to see results.
function approx(a: number, b: number, eps = 1e-6) { return Math.abs(a - b) < eps; }
export function runTests() {
  // Test 1: pickTier monotonic
  console.assert(pickTier(12).net <= pickTier(18).net && pickTier(60).net >= pickTier(24).net, "pickTier should be monotonic by months");

  // Test 2: payout mode produces constant monthly payout = amount * rate/12
  const amt = 1_000_000; const months = 12; const calcPayout = useCalc(amt, months, "payout");
  console.assert(approx(calcPayout.monthlyPayout, amt * (pickTier(months).net/12)), "Monthly payout mismatch");

  // Test 3: reinvest grows principal strictly when rate > 0
  const reinv = useCalc(amt, months, "reinvest");
  console.assert(reinv.finalAmount > amt, "Reinvest should increase principal");

  // Test 4: 0 months edge case
  const zero = (() => { const m = 0; const t = pickTier(12); const r = t.net/12; return { m, t, r }; })();
  console.assert(zero.r > 0, "Rate per month should be > 0 for default tier");
  console.log("All tests passed (if no assertions failed).");
}

if (typeof window !== "undefined") {
  try { runTests(); } catch (e) { console.warn("Tests error:", e); }
}
